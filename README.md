...
```bash
python build_ics.py && git add -A && git commit -m "更新名額" && git push
```

或不開本機，讓 GitHub 雲端跑：`gh workflow run "更新補位行事曆"`（名額有變才 commit）。

**Google 抓外部 ICS 的頻率自己決定，通常好幾小時到一天**，推上去之後行事曆不會馬上變。
想馬上看到就取消訂閱，改用 `hiking.ics?v=N`（N 換個新數字）重新訂閱。
看到剩 1–2 的直接點進商品頁確認，不要信行事曆上的數字。
