...
# 商品標題含這些字就整個商品跳過。前兩個是課程不是登山團，後兩個是使用者指定不要看的路線。
#   課        368 的裝備選購×輕量化打包課、離線地圖戶外實戰課
#   行進技巧  368 的登山行進技巧初階班、Day2 實戰班
#   雪山東峰  368 的雪山東峰兩天一夜登山團（不會誤中荒野的「雪山主東」）
#   眠月線    兩站都有，一起排掉
# 合歡四峰高山入門班是刻意留著的：名字有「班」但實際是兩天一夜上合歡群峰。
# 海外團不靠標題排除，走分類（見 SITES 的 exclude_categories）。
# 改這裡之後，下次回報更新結果時要主動把目前的排除清單列給使用者看。
SKIP_TITLE = ("課", "行進技巧", "雪山東峰", "眠月線")
...
def parse_dates(size_name, today):
    ...
    s = (size_name.replace("／", "/").replace("（", "(").replace("）", ")")
         .replace("＊", "*").replace("．", "."))
    # D0 是前一晚集合日，不是梯次本身的日期，要整段拿掉。這步一定要在點號轉斜線之前：
    # 否則「D0.10/2」會先被當成日期改寫成「D0/10/2」，這條規則就對不上，
    # 接著 DATE_HEAD 會抓到「0/10」變成月份 0。
    ...
    # 368 有些梯次用點號寫日期：「2026.10.09（五） - 10.11（日）」，統一成斜線。
    s = re.sub(r"(\d{4})\.(\d{1,2})\.(\d{1,2})", r"\1/\2/\3", s)
    s = re.sub(r"(?<!\d)(\d{1,2})\.(\d{1,2})(?!\d)", r"\1/\2", s)
    s = s.split("*")[0]
